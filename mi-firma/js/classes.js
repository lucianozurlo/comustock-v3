...
class cs_Before_After {
  constructor ($obj = null) {
    ...
    this.$el = {
      $wrap: $obj,
      $before: jQuery (
        '<div class="cs-before-after-img cs-before-img"/>'
      ).appendTo ($obj),
      $after: jQuery (
        '<div class="cs-before-after-img cs-after-img-wrap"/>'
      ).appendTo ($obj),
      $divider: jQuery (
        '<div class="cs-before-after-divider">\
    ...
    this.$el.$after.children ('img').wrap ('<div class="cs-after-img"/>');
    this.$el.$after
      .children ('.cs-after-img')
      .css (
        'background-image',
...
const baw_axis = new cs_PanAxis (10);
...
class cs_Menu {
  constructor (_core = null) {
    ...
    this.core.$dom.header.append ('<span class="cs-active-menu-ind"></span>');
    ...
    // Create Mobile Menu
    this.mobile_menu = new cs_Mobile_Menu (this.$nav);
  }
  layout () {
    const _self = this;
    let active_pos = '-100%';
...
class cs_Mobile_Menu {
  constructor ($obj) {
    ...
    this.$nav = jQuery ('<nav class="cs-mobile-nav"/>').appendTo (this.$body);
    if (this.$el.parents ('header').hasClass ('is-frosted')) {
      this.$nav.addClass ('is-frosted');
    }
    this.$menu = jQuery ('<ul class="cs-mobile-menu"/>').appendTo (this.$nav);
    this.$el.children ('ul').children ().each (function () {
      jQuery (this).clone ().appendTo (_self.$menu);
    });
    this.$overlay = jQuery ('<div class="cs-mobile-menu-overlay"/>').appendTo (
      this.$body
    );
    ...
    // Events
    jQuery ('.cs-mobile-menu-toggler').on ('click', function () {
...
class cs_Infinite_List {
  constructor ($obj = null, cfg = {}) {
    ...
    // Hover Event
    this.$el
      .on ('mouseenter', '.cs-infinite-list-item', function () {
        ...
        if ('video' === $preview.attr ('data-type')) {
          jQuery (this)
            .children ('.cs-infinite-list-preview')
            .children ('video')[0]
            .play ();
          $preview.children ('video')[0].play ();
        }
      })
      .on ('mouseleave', '.cs-infinite-list-item', function () {
  ...
  init () {
    ...
    // DOM
    this.$list = this.$el.children ('.cs-infinite-list');
    this.$bg_wrap = jQuery ('<div class="cs-infinite-list-bgs"/>').appendTo (
      this.$el
    );
    this.$scroll = jQuery ('<div class="cs-infinite-scroll"/>').appendTo (
      this.$el
    );
    this.$cloned_list = jQuery (
      '<div class="cs-infinite-scroll-inner"/>'
  ...
  clone_items () {
    ...
    this.$list.children ().each (function (i) {
      ...
      if (_self.first_circle) {
        let $preview = $cloned.children ('.cs-infinite-list-preview');
        ...
        if ('video' === type) {
          ...
          _self.$bg_wrap.append (`
                    <div class="cs-l-bg-preview-item${i === 0 ? ' is-active' : ''}" data-type="video" data-id="${i}" style="background-image:url(${this_src})">
                        <video src="${this_src}" webkit-playsinline="true" playsinline="true" muted loop></video>
                    </div>`);
        } else {
          $preview.css ('background-image', 'url(' + this_src + ')');
          _self.$bg_wrap.append (
            `<div class="cs-l-bg-preview-item${i === 0 ? ' is-active' : ''}" data-type="image" data-id="${i}" style="background-image:url(${this_src})"/>`
          );
        }
      }
    });
...
class cs_Marquee {
  constructor ($obj = null) {
    ...
    this.$inner = this.$marquee.children ('.cs-marquee-inner');
    this.$content = this.$inner.children ();
    this.width = this.$inner.find ('.cs-marquee-list').width ();
  ...
  init () {
    this.$inner.wrapInner ('<div class="cs-marquee-inner-wrap"/>');
    this.$inner_cont = this.$inner.children ('.cs-marquee-inner-wrap');
...
class cs_Masked {
  constructor ($obj = null, cfg = {}) {
    ...
    this.cfg = Object.assign (
      {
        media_selector: '.cs-masked-media',
        content_selector: '.cs-masked-content',
        dbr: 24,
        ibr: 32,
      },
      cfg
...
class cs_LWP {
  constructor ($obj = null) {
    ...
    this.$media = jQuery (
      '<div class="cs-lwp-media" data-appear="fade-right"/>'
    ).prependTo (this.$el);
    this.count = this.$list.children ('.cs-lwp-item').length;
    this.active = 0;
    this.prev = 1;

    this.$list.children ('.cs-lwp-item').each (function (i) {
